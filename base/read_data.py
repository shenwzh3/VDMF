...
def draw_mat(rate):
    ...
    for line in lines:
        ...
        if(int(line_data[2]) >= 4):
            ui_mat[int(line_data[0]) - 1,int(line_data[1]) - 1] = 1.
...
def get_recall(w1,w2):
    ...
    sum_like = np.sum(ui_mat,axis = 1,keepdims = True) # shape = (943,1)
    recall_dict = {}
    for K in [50,100,150,200,250,300]:
        topK_like = sorted_rate[:] #the top-K likes of all the users, shape = (943,K)
        for i in range(len(topK_like)):
            topK_like[i] = topK_like[i][0:K]
        sum_recall = 0
        for i in range(943):
            num_like = 0
...
# to test the recall, use the user 0 - 9
def read_data_recall_test():
    ui_mat = draw_mat('100')

    user_data = ui_mat[0:10, :]
    item_data = ui_mat.T
    ...
    sideinfo = json.loads(json_text)
    user_info = np.array(sideinfo['user'][0:10], dtype = np.float32)
    item_info = np.array(sideinfo['item'], dtype = np.float32)
